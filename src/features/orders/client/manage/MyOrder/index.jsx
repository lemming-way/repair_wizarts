...
import { UserRole, BusinessModel, useUser, useUsersByIds } from '../../../../../state/user';
import { useOrdersByIds, useCancelOrder, useAcceptOffer, OrderStatus, orderStatusString } from '../../../../../state/order';
import { useServices } from '../../../../../state/site-data';
import commonStyle from '../../../../../components/Service/ServiceDetail.module.scss';
...
function MyOrder() {
  ...
  const serviceName = currentOrder?.serviceId ? services[currentOrder.serviceId]?.name : text('Unknown Service');
                    ...
                    <table>
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                            {contractorProfile.services?.length > 0
                              ? contractorProfile.services.map(srvId => services[srvId]?.name).filter(Boolean).join(', ')
                              : text('not specified')}
                          </td>
                        </tr>
                      </tbody>
